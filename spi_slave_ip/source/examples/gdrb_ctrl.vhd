...
-- pragma translate_on
    ...
    FOC_SDO : out std_logic := 'Z';                 -- This will go back to GDPB via a pin on the camera link tx to GHDB
    ...
    ---.....Illumination DAC's
    CIS_ILLUM_DAC_SYNC1_BAR : out std_logic := '0';  -- CIS DAC's are write only -- These DAC's are the same part number as 'Begalbone SPI to illumination DAC's' below
...
testbench_gen : if testbench_mode generate
    ...
    -- pragma translate_off
        --Testbench signals for begalbone SPI testing
        reg_map_array_from_pins_s <= bb_reg_map_array_from_pins; -- : in mem_array_t(0 to (2**SPI_ADDRESS_BITS)-1, SPI_DATA_BITS-1 downto 0);
        bb_reg_map_array_to_pins <= reg_map_array_to_pins_s; -- : out mem_array_t(0 to (2**SPI_ADDRESS_BITS)-1, SPI_DATA_BITS-1 downto 0);
    -- pragma translate_on
    ...
    ---GHDB master SPI from GHDB to.....
    ---......GDRB_DPMUX FPGA (straight through from FOC_xx ports unless there are issues)
    CISMUX_SDI <= TO_UX01(FOC_SDI);           -- Pass GHDB input SPI signals on to gdrb_dpmux FPGA
    cismux_sclk <= TO_UX01(FOC_SCLK);         -- Pass GHDB input SPI signals on to gdrb_dpmux FPGA
    cismux_sen_bar <= TO_UX01(FOC_SMODE_BAR); -- Pass GHDB input SPI signals on to gdrb_dpmux FPGA
    FOC_SDO <= TO_UX01(cismux_sdo);           -- Output tri-state SPI MISO from this FPGA to go back to Griffin data path board GDPB (this can be driven from SPI interfaces inside this FPGA too)

----THIS ACTUALLY  GOES INTO THE DP_MUX FPGA - an SPI interface will be fitted but that will be for AFE and DAC 
-----------------------------------------------------------------------------------
...
--            MEM_ARRAY_T_INITIALISATION => bs_mem_array_t_initalised_c           -- Function that populates this constant in 'spi_xxxx_pkg'
--            )
--    Port map(  
--            clk => CLK60M_S,                                                    -- : in std_logic;
--            reset => reg_map_reset_s,                                           -- : in std_logic;
